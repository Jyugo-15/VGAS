#针对25号一些没测成功的跑的，顺便加上信息一个ckpt的测试
...
BEST_OF_N=8         # <=1 表示 BC
N_ACTION=20         # --n-action-steps (实际执行步数)
# Q_AGG="mean"      # --critic-q-agg-eval，如不需要可设为空字符串
EVAL_BATCH=50       # --eval-batch-size
N_EPISODES="$EVAL_BATCH"  # --n-episodes（默认等于 batch）
SEEDS=(0 42 1234 124 410 2000)        # 按需添加多个 seed 
TASK_ID=""          # 为空则评估所有任务；设置为单个任务 id（如 0）
